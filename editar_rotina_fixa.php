...
// ===== LIDAR COM GET (EXIBIR FORMULÁRIO) =====
...
// ===== LIDAR COM POST (FORMULÁRIO HTML) =====
if ($_SERVER['REQUEST_METHOD'] === 'POST' && strpos($contentType, 'application/json') === false) {
...
<body>
    <div class="container">
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST">
                    <!-- ID da rotina em hidden field para POST formulário -->
                    <input type="hidden" name="id" value="<?php echo isset($rotinaId) ? $rotinaId : ''; ?>">
                    
                    <div class="mb-3">
                        <label class="form-label">Nome da Rotina</label>
                        <input type="text" name="nome" class="form-control" value="<?php echo isset($rotina) ? htmlspecialchars($rotina['nome']) : ''; ?>" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Horário Sugerido (opcional)</label>
                        <input type="time" name="horario" class="form-control" value="<?php echo (isset($rotina) && $rotina['horario_sugerido']) ? substr($rotina['horario_sugerido'], 0, 5) : ''; ?>">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Descrição (opcional)</label>
                        <textarea name="descricao" class="form-control" rows="3"><?php echo isset($rotina) ? htmlspecialchars($rotina['descricao']) : ''; ?></textarea>
                    </div>
