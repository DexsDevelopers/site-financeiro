...
(function () {
    ...
    window.ONBOARDING_STEPS = {
        ...
        'gestao_empresas.php': function (exists, isMobile) {
            const steps = [];
            intro(steps, exists, 'Gestão de Empresas', 'Bem-vindo ao seu ecossistema de negócios. Gerencie múltiplas marcas, finanças e marketing em um só lugar.');
            safePush(steps, exists, step('.btn-custom-red', 'Nova Empresa', 'Cadastre uma nova empresa ou unidade de negócio.', 'right'));
            safePush(steps, exists, step('.ge-card', 'Acessar Empresa', 'Clique em um card para entrar e gerenciar os detalhes da empresa.', 'bottom'));
            safePush(steps, exists, step('.ge-tabs', 'Navegação por Módulos', 'Alterne entre Finanças, Tarefas, Ideias, Marketing e mais dentro da empresa selecionada.', 'bottom'));
            safePush(steps, exists, step('.ge-widget-grid', 'Indicadores Rápidos', 'Acompanhe o desempenho financeiro e tarefas pendentes num relance.', 'top'));
            return steps;
        }
    };
})();
